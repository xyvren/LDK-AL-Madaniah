...
<br>
<div align="center">
<img src="https://img.shields.io/badge/version-v2.3.0-blue" />
<img src="https://img.shields.io/badge/laravel-8.x-red" />
<img src="https://img.shields.io/badge/php-%3E%3D8.2-777BB4" />
<img src="https://img.shields.io/badge/license-LDK Al-Madaniah-green" />
<img src="https://img.shields.io/badge/contributors-11-brightgreen" />
</div>

## About

LDK Al-Madaniah UBB Web App is a comprehensive web platform for **Lembaga Dakwah Kampus (LDK) Syahid UBB**. It serves as both a public-facing website and a role-based admin panel (CMS) for managing organizational content, services, and member activities.
...
- **KTA LDK Al-Madaniah** — Digital member ID card (Kartu Tanda Anggota) generation
...
| `ExpireStaleQrisDonations` | Expires QRIS donation records that passed their BisaTopup expiry window |

## Version History

| Version | Date | Changes |
| :---: | :---: | --- |
| `2.3.0` | `2026-07-16` | <ul><li>**Celengan Syahid — Withdrawal & Disbursement Module** — Complete fund withdrawal flow: create draft → inquiry rekening → 2FA confirm → execute via Bisabiller API → callback webhook; includes per-campaign Finance page with QRIS/cash balance summary and withdrawal history</li><li>**Balance Report** — Real-time Bisabiller wallet reconciliation page comparing actual wallet balance vs. expected DB balance; settlement-pending detection excludes in-transit QRIS payments to prevent false deficit alerts; Balance History ledger with unified payment + disbursement log and running balance column</li><li>**2FA Security for Withdrawal** — TOTP-based Two-Factor Authentication (Google Authenticator) required to execute any withdrawal; anti-replay via `verifyKeyNewer`, rate limiting (5 attempts per 5 min), whitelist access via `TWO_FA_ALLOWED_USERS`; 2FA setup page with QR code; full audit trail of 2FA events</li><li>**Dynamic Form — Video Field** — New video embed field type for YouTube/iframe in form builder and public form renderer</li><li>**Dynamic Form — Date, Time & Dropdown Fields** — Styled date picker, time picker, and dropdown select fields with consistent UI</li><li>**Dynamic Form — Live Preview** — Real-time desktop + mobile live preview panel in form builder; preview updates as fields are added/reordered</li><li>**Dynamic Form — Section Routing** — Conditional section routing based on dropdown or multiple-choice answers; sections can branch to different next sections based on selected option</li><li>**Dynamic Form — Form Analytics** — New analytics page per form showing response breakdown per field (bar/pie charts) and summary statistics</li><li>**Popup — Gemilang Perunggu Theme** — New welcome popup theme "Gemilang Perunggu" replacing the previous Milad theme; message content managed via `PopupMessage` model with admin AJAX endpoint</li><li>**Donation — QRIS Fee Breakdown** — Admin fee (MDR 1%) shown as a separate line on the donation form with dark mode support; CEIL formula prevents Rp 1 wallet shortfall on rounding</li><li>**Donation — Expired QRIS Handling** — `ExpireStaleQrisDonations` command scheduled via cron to auto-expire QRIS records past their Bisabiller expiry window</li><li>**Security — Callback Authentication** — Disbursement callback protected by URL path secret (Bisabiller rejects query strings); donation callback signature always enforced on `live` env; IP allowlist middleware (`VerifyBisabillerIp`) on both webhook routes</li><li>**Security — Atomic Withdrawal Execution** — Status flipped DRAFT→PENDING atomically before Bisabiller API call; prevents double-spend race condition when two admins execute simultaneously; balance re-checked post-flip and rolled back to DRAFT if insufficient</li><li>**Security — Brute Force Defense** — Throttle `5/min` on withdrawal execute route; throttle added to donation status, payment proof, and gateway redirect endpoints; 2FA rate limiting with 5-attempt lockout</li><li>**Security — XSS & Info Disclosure Fixes** — `link_pj` URL validated for `https://` scheme before rendering in href; exception messages replaced with generic user-facing errors; debug log entries (JWT token, login response body) removed from production logs; error log detail enriched with exception class, URL, and context fields</li><li>**Bug Fix — Balance History Settling Badge** — Fixed "Settling…" badge incorrectly appearing on already-settled payments when multiple consecutive QRIS payments exist; settling attribution now processed newest-first, accumulated against actual wallet gap, and stops once gap is fully attributed</li></ul> |
| `2.2.0` | `2026-06-13` | <ul><li>**Celengan Syahid — Full Revamp** — Complete redesign of the donation flow: campaign listing, detail page, donation form, and payment status page with improved UI, dark mode support, and mobile responsiveness</li><li>**BisaTopup QRIS Gateway** — New QRIS payment method via BisaTopup integration; QRIS QR code rendered on payment status page with auto-polling every 5 seconds; payment status URL automatically saved as `payment_link` for admin tracking</li><li>**reCAPTCHA — Google Cloud Migration** — Migrated reCAPTCHA keys to Google Cloud console (v2 Checkbox challenge, domain `ubb.ac.id`); server-side verification via classic `api/siteverify`; togglable via `RECAPTCHA_ENABLED`</li><li>**Admin Donation — Create & Edit Forms** — New admin-side donation form with CASH payment method support (for manual/offline donations); Payment Method and Payment Status dropdowns with Select2 styling</li><li>**Admin Donation — Payment Method Column** — New `Payment Method` column in donation index with dropdown filter; shows gateway method (QRIS, EWALLET, CASH, etc.)</li><li>**Admin Donation Detail — Refactored View** — Donation detail page redesigned to match the admin comment view style; added Edit button next to Back; payment link shown as prominent button with URL below</li><li>**Admin celengan-syahid restructure** — Separated inline CSS and JS into component files across audit-log, dashboard, and donation folders following the job-queue-log pattern</li><li>**Landing-page celengan-syahid restructure** — Extracted payment-status scripts to `_payment-status-scripts.blade.php` component</li><li>**Donation Form — UI Error Display** — Server-side validation errors (including reCAPTCHA failures) now shown in the UI as an alert box above the form and inline below the captcha widget</li><li>**Donation Preset Nominations** — Added small preset buttons Rp1.000, Rp2.000, Rp5.000; minimum donation reduced from Rp10.000 to Rp1.000</li><li>**Audit Log UI Cleanup** — Removed Filter and Clear buttons; filtering now auto-triggers on select2 change and debounced search input</li><li>**QRIS Scan Dark Mode** — QRIS QR code display on payment status page supports dark mode</li><li>**Prevent Duplicate WA Notification** — Fixed double WhatsApp invoice notification when BisaTopup callback fires multiple times</li><li>**Donation Throttle** — Rate limit increased from 10 to 30 submissions per minute</li><li>**Queue Job Attempts Column** — Widened `attempts` column in `tr_job_queues` to prevent overflow on high-retry jobs</li><li>**Admin views — All English** — All admin celengan-syahid views now use English labels (donation show, campaign form categories, donation form)</li></ul> |
